...
public class FolderBuilder {
	...
	public JSONArray build(String id) 
	{
        ...
        //查询文件
        String where = String.format("CHARINDEX('%s',f_pathRel)>0 and f_fdTask=0 and f_deleted=0", pathRoot+"/");
        DBConfig dc = new DBConfig();
        if(dc.m_db.equals("mysql"))
        {
        	where = String.format("locate('%s',f_pathRel)>0 and f_fdTask=0 and f_deleted=0", pathRoot+"/");
        }
        else if(dc.m_db.equals("oracle"))
        {
        	where = String.format("instr(f_pathRel,'%s')>0 and f_fdTask=0 and f_deleted=0", pathRoot+"/");
        }
