...
function HttpUploaderMgr()
{
	...
	this.Config = {
		...
		//文件夹操作相关
        , "UrlFdCreate"		: page.path.root+"up6/fd_create"
        , "UrlFdComplete"	: page.path.root+"up6/fd_complete"
        , "UrlFdDel"	    : page.path.root+"up6/fd_del"
        //文件操作相关
        , "UrlCreate"		: page.path.root+"up6/f_create"
        , "UrlPost"			: page.path.root+"up6/f_post"
        , "UrlProcess"		: page.path.root+"up6/f_process"
        , "UrlComplete"		: page.path.root+"up6/f_complete"
        , "UrlList"			: page.path.root+"up6/f_list"
        , "UrlDel"			: page.path.root+"up6/f_del"
	    //x86
        , ie: {
		...
		, ui: {
            icon: {
                upFile: page.path.root + "static/js/16/upload.png",
                upFolder: page.path.root + "static/js/16/folder.png",
                paste: page.path.root + "static/js/16/paste.png",
                clear: page.path.root + "static/js/16/clear.png",
                file: page.path.root + "static/js/file.png",
                folder: page.path.root + "static/js/folder.png",
                stop: page.path.root + "static/js/stop.png",
                del: page.path.root + "static/js/del.png",
                post: page.path.root + "static/js/post.png"
            }
        }
        , errCode: {
            "0": "发送数据错误"
	...
	this.ui={
		tmp:{file:null,folder:null/**文件夹上传项模板 */},
		btn:{setup:null/**控件安装按钮 */},
		poster:null/**上传面板 */
	};
	...
	this.GetHtmlContainer = function()
	{
        ...
        com += '<div name="post_panel">\
					<div name="post_head" class="files-toolbar">\
						<span class="tool-btn" name="btnAddFiles"><img name="upFile"/>传文件</span>\
						<span class="tool-btn" name="btnAddFolder"><img name="upFolder"/>传目录</span>\
	...
	this.initUI = function (dom)
	{
	    ...
	    this.ui.poster    = post_body;
	    this.ui.tmp.file  = panel.find('div[name="fileItem"]');
	    this.ui.tmp.folder = panel.find('div[name="folderItem"]');
        this.ui.btn.setup  = panel.find('span[name="btnSetup"]').click(function () {
	...
	*/
	...
	this.addFileLoc = function(fileLoc)
	{
		...
		var ui = _this.ui.tmp.file.clone();//文件信息
		_this.ui.poster.append(ui);//添加文件信息
	...
	this.addFolderLoc = function (json)
	{
		...
		var ui = this.ui.tmp.folder.clone();//文件夹信息
		this.ui.poster.append(ui);//添加到上传列表面板
		ui.css("display", "block");
