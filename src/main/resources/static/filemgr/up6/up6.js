...
function HttpUploaderMgr()
{
	...
	this.Config = {
		...
		//文件夹操作相关
		, "UrlFdCreate"		: page.path.root + "filemgr/fd_create"
		, "UrlFdComplete"	: page.path.root + "up6/fd_complete"
		, "UrlFdDel"	    : page.path.root + "up6/fd_del"
		//文件操作相关
		, "UrlCreate"		: page.path.root + "filemgr/f_create"
	...
	this.initUI = function (dom)
	{
        ...
        this.ui.poster = dom.find(this.Config.ui.list);
        this.ui.tmp.file = dom.find(this.Config.ui.file);
        this.ui.tmp.folder = dom.find(this.Config.ui.folder);
        ...
        this.ui.btn.setup = dom.find('span[name="btnSetup"]').click(function () {
	...
	*/
	...
	this.addFileLoc = function(fileLoc)
	{
		...
		var tmp = this.ui.tmp.file.clone();//文件信息
		tmp.css("display", "block");
        this.ui.poster.append(tmp);//添加文件信息
	...
	this.addFolderLoc = function (json)
	{
        ...
        var tmp = this.ui.tmp.file.clone();//文件夹信息
		tmp.css("display", "block");
		this.ui.poster.append(tmp);//添加到上传列表面板
        var ui = this.find_ui(tmp);
        ui.ico.file.hide();
        ui.ico.folder.show();
